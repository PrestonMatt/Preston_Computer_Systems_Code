...
int main()
{
  ...
  //place player1 & 2 at pos = 0 or first position.
  player_t p1, p2;
  p1.pos = f;
  p2.pos = f;
  p1.win_condition = 1;
  ...
  while(p1.win_condition || p2.win_condition)
  {

    //print space and roll:
    node_t* newSpace1 = p1.pos;
    node_t* newSpace2 = p2.pos;
    square_t *aSpace1 = newSpace1->item;
    square_t *aSpace2 = newSpace2->item;
    printf("Player 1 on %s and ", toString(aSpace1));
    printf("Player 2 on %s and \n", toString(aSpace2));

    int rn1 = randomNumber(4);
    char* spinnedColor1 = convertToColor(rn1);
    //sleep(1);
    int rn2 = randomNumber(4);
    char* spinnedColor2 = convertToColor(rn2);
    //sleep(1);

    printf("Player 1 spinned a %s, ", spinnedColor1);//"%d, aka %s\n", rn, spinnedColor);
    printf("Player 2 spinned a %s,\n", spinnedColor2);

    //node_t* check = p1.pos;

    //move:
    p1.pos = move(spinnedColor1, newSpace1);
    newSpace1 = p1.pos;
    p2.pos = move(spinnedColor2, newSpace2);
    newSpace2 = p2.pos;

    //follow special instructions (sans winning) on board:
    p1.pos = specials(newSpace1);
    p2.pos = specials(newSpace2);
    //if(check != newSpace){ printf("On %s and ", toString(aSpace)); }

    //check if the space you're on is winning;
    p1.win_condition = checkWin(p1.pos);
    p2.win_condition = checkWin(p2.pos);
    //printf("%d win\n", p1.win_condition);
    //printf("%d win\n", p2.win_condition);

    if(p1.win_condition == 0 && p2.win_condition != 0)
    {
      printf("\n");
      printf("Player 1 Won!\n");
      break;
    }
    else if(p1.win_condition != 0 && p2.win_condition == 0)
    {
      printf("\n");
      printf("Player 2 Won!\n");
      break;
