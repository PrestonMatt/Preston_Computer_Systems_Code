  ...
  while (boolStopNotFound)
  {
    ...
    while ((ch = getchar()) != ' ')
    {
      ...
      if (ch == '\b')
      {
        ...
        printf("Why would you type that?");
        return 0;
      }
      if (ch == '\r')
      {
      ...
      else if (ch == '\n') //When the line is done, so are we with the last word!
      {
        break;
      }
    }   /* while _getch() */
    ...
    *wordUnderConstruction = '\0';  /* finish word under construction */
    ...
    if (IsWordEqualToStop(wordStr))
    {
      boolStopNotFound = 0;
      printf("Stopping...\n");
    } /* OK SO We're reading words and now we gotta add them to the linked list:*/
    else
    {
